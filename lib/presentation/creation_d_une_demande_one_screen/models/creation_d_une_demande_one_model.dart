import 'package:bgpt/data/models/selectionPopupModel/selection_popup_model.dart';
import '../../../core/app_export.dart';
import 'creationdemandeone_item_model.dart';

/// This class defines the variables used in the [creation_d_une_demande_one_screen],
/// and is typically used to hold data that is passed between different parts of the application.
class CreationDUneDemandeOneModel {
  Rx<List<SelectionPopupModel>> dropdownItemList = Rx([
    SelectionPopupModel(
      id: 1,
      title: "Item One",
      isSelected: true,
    ),
    SelectionPopupModel(
      id: 2,
      title: "Item Two",
    ),
    SelectionPopupModel(
      id: 3,
      title: "Item Three",
    )
  ]);

  Rx<List<SelectionPopupModel>> dropdownItemList1 = Rx([
    SelectionPopupModel(
      id: 1,
      title: "Item One",
      isSelected: true,
    ),
    SelectionPopupModel(
      id: 2,
      title: "Item Two",
    ),
    SelectionPopupModel(
      id: 3,
      title: "Item Three",
    )
  ]);

  Rx<List<CreationdemandeoneItemModel>> creationdemandeoneItemList = Rx([
    CreationdemandeoneItemModel(
        uploadSomeFiles: "Importer le releve de compte".obs),
    CreationdemandeoneItemModel(
        uploadSomeFiles: "Importer la facture SNDE".obs),
    CreationdemandeoneItemModel(
        uploadSomeFiles: "Importer la facture Somelec".obs),
    CreationdemandeoneItemModel(uploadSomeFiles: "Importer votre sejour".obs)
  ]);
}
